...
Runs on free tiers end to end — Vercel + Neon (Postgres) + Upstash (Redis).

```bash
./scripts/deploy.sh   # login, provision Postgres + Redis, deploy, migrate, seed
```

Step-by-step in [DEPLOY.md](DEPLOY.md), or one click:
